...
import { getMemberFavoriteList } from "../../../modules/commerble-nextjs-sdk/client/api";
...
function useCommerble(query) {
    ...
    const [data, mutate] = useCommerbleState(() => getMemberFavoriteList(page), [page]);
    ...
    return { 
        data: data?.type === 'member/favoritelist' ? data : null, 
        mutate,
    };
}


export default function MyFavoriteListPage({query}) {
                ...
                <div className="form-body">
                    ...
                    {cb.data.items.map(item => 
                        <CartLine 
                            ...
                            href={`/mypage/favorites/${item.product.id}`}
                            hiddenActions 
                            qty={undefined}/>)}
                    <hr />
                    {cb.data.paging.current < cb.data.paging.maxPage - 1 &&
                        <Link href={`/mypage/favorites/?page=${cb.data.paging.current+1}`}>
                            <Button
                                looks="default"
                                className="w-full"
