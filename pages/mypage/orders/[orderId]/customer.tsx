...
import { getOrderHistory, modifyCustomer } from "../../../../modules/commerble-nextjs-sdk/client/api";
...
export default function MyOrderPage({params}) {
    ...
    const onSubmit = async (data) => {
        setLoading('submit');
        try {
            const result = await modifyCustomer(data);
            if (result.type === 'order/historycustomer') {
            ...
            <div className="layout-2col__col x-center py-16">
                {cb.data&&
                    <form className="form-body" onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <div className="field">
                            <label>メールアドレス</label>
                            <input {...register("orderCustomer.emailAddr", { required: true })} autoFocus={true}/>
                            {errors?.orderCustomer?.emailAddr && <span>This field is required</span>}
                        </div>
                        <hr/>
