...
import { getSiteActivateRequest, requestActivation } from "../../../modules/commerble-nextjs-sdk/client/api";

function useCommerble() {
    const [data, mutate] = useCommerbleState(() => getSiteActivateRequest());
    return { 
        data: data?.type === 'site/activaterequest' ? data : null, 
        mutate
    };
}
...
export default function ReactivatePage() {
    ...
    const onSubmit = async (data) => {
        setLoading('submit');
        try{
            const result = await requestActivation(data, cb.data.token);
            if (result.type === 'site/activaterequest') {
                cb.mutate(result);
            }
        }
        finally{
